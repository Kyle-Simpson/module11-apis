...
movie <- c("darkrai", "infernape", "cyntaquil", "torchic")

# Make a variable called pokemon url that holds your favorite pokemon's url 
pokemon.url <- paste0(base.url, "darkrai")
...
# What do you think is the diffrence between the two outputs above? Why are they diffrent?
    # Poke.data is not technically a data frame because it is full of data frames.
    # Poke.data$moves is a data frame
...
poke.moves$move.type <- "lit"  # Not correct, but still valid
poke.moves$move.rating <- 1:5  # Not correct, but I'm lazy


# Using dplyr order the moves by rank. Higher ranking moves should be on the top of the dataframe. 
poke.moves <- arrange(poke.moves, -move.rating)
View(poke.moves)
